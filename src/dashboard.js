...
export async function buildDashboardStatus(env) {
  ...
  const deviceViews = (Array.isArray(devices) ? devices : []).map(rawDevice => {
    ...
    return {
      ...
      apiFaultActive: !!dState.apiFaultActive,
      lastApiFaultReason: dState.lastApiFaultReason || null,
      lastApiFaultAlertAt: dState.lastApiFaultAlertAt || null,
      lastApiFaultRecoveryAt: dState.lastApiFaultRecoveryAt || null,
  ...
  const summary = {
    ...
    devicesWithFault: deviceViews.filter(d => d.sensorFaultActive || d.apiFaultActive).length,
